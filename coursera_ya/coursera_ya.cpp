...
public:
	Paginator(Iterator begin, Iterator end, size_t page_size) {
		...
		for (auto it = begin; it != end; ++it) {
			if (count == 0) {
				first = it;
			}else if(count == page_size || it == prev(end)){
				pages.emplace_back(MakeRange(fisrt, it));
			}
		}
	}

	vector<IteratorRange<Iterator>>::iterator begin() const {
		return pages.begin();
	}

	vector<IteratorRange<Iterator>>::iterator end() const {
		return pages.end();
	}
...
template <typename C>
Paginator<C::iterator> Paginate(C& c, size_t page_size) {
	// Реализуйте этот шаблон функции
	Paginator<C::iterator> p(c.begin(), c.end(), page_size);
	return p;
}
